...
import { getBlacklistedWalletById } from 'apiSdk/blacklisted-wallets';
...
function BlacklistedWalletViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Blacklisted Wallet Detail View
          </Text>
          {hasAccess('blacklisted_wallet', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/blacklisted-wallets/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              {hasAccess('wallet', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/wallets/view/${data?.wallet?.id}`}>
                      {data?.wallet?.address}
                    </Link>
                  </Text>
                </Flex>
